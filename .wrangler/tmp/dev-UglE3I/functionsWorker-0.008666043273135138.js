...
// .wrangler/tmp/bundle-vHglkk/checked-fetch.js
...
// .wrangler/tmp/bundle-vHglkk/middleware-loader.entry.ts
var import_checked_fetch16 = __toESM(require_checked_fetch());

// wrangler-modules-watch:wrangler:modules-watch
var import_checked_fetch = __toESM(require_checked_fetch());

// .wrangler/tmp/bundle-vHglkk/middleware-insertion-facade.js
var import_checked_fetch14 = __toESM(require_checked_fetch());

// .wrangler/tmp/pages-7JAahO/functionsWorker-0.008666043273135138.mjs
...
var require_checked_fetch2 = __commonJS2({
  "../.wrangler/tmp/bundle-yxNnzP/checked-fetch.js"() {
    var urls = /* @__PURE__ */ new Set();
    function checkURL(request, init) {
      const url = request instanceof URL ? request : new URL(
        (typeof request === "string" ? new Request(request, init) : request).url
...
async function handleSendMail({ request, env, waitUntil }) {
  ...
  const data = body && body.data && typeof body.data === "object" ? body.data : Object.fromEntries(Object.entries(body || {}).filter(([k]) => k !== "formType"));
  ...
  if (!data.submittedAt) data.submittedAt = (/* @__PURE__ */ new Date()).toISOString();
  if (!data.source) data.source = "ivorytusk-website";
  let rendered;
  let replyTo;
  try {
    rendered = renderForType(formType, data);
    replyTo = typeof data.email === "string" ? data.email : void 0;
  } catch (err) {
    return jsonResponse({ ok: false, error: err.message }, 400, cors);
  }
  const recipients = (env.NOTIFY_EMAILS || "contact@ivorytusk.co.in").split(",").map((s) => s.trim()).filter(Boolean);
...
__name(handleSendMail, "handleSendMail");
__name2(handleSendMail, "handleSendMail");
...
__name2(corsHeadersFor, "corsHeadersFor");
...
__name(isAllowedRequest, "isAllowedRequest");
__name2(isAllowedRequest, "isAllowedRequest");
...
__name(renderForType, "renderForType");
__name2(renderForType, "renderForType");
async function sendMail(env, msg) {
  ...
  const encoder = new TextEncoder();
  let buffer = "";
  const readLine = /* @__PURE__ */ __name2(async () => {
    while (true) {
  ...
  const readReply = /* @__PURE__ */ __name2(async (expected) => {
    const lines = [];
    while (true) {
  ...
  const send = /* @__PURE__ */ __name2(async (line) => {
...
__name2(extractEmail, "extractEmail");
...
\r
` + dotStuff(msg.text) + "\r\n";
  const htmlPart = `--${boundary}\r
Content-Type: text/html; charset=UTF-8\r
Content-Transfer-Encoding: 8bit\r
\r
` + dotStuff(msg.html) + "\r\n";
  const closing = `--${boundary}--\r
`;
...
function humanize(key) {
  return key.replace(/[_-]+/g, " ").replace(/([a-z])([A-Z])/g, "$1 $2").replace(/\s+/g, " ").trim().replace(/\b\w/g, (c) => c.toUpperCase());
}
__name(humanize, "humanize");
__name2(humanize, "humanize");
function formatValue(v) {
  if (v === null || v === void 0 || v === "") return "Not provided";
  if (Array.isArray(v)) return v.length ? v.map((x) => String(x)).join(", ") : "Not provided";
  if (typeof v === "object") return JSON.stringify(v);
  return String(v);
}
__name(formatValue, "formatValue");
__name2(formatValue, "formatValue");
function renderGenericForm(formType, data) {
  ...
  const submitterName = typeof data.name === "string" ? data.name : typeof data.fullName === "string" ? data.fullName : "";
  const subjectLead = submitterName ? ` \u2014 ${submitterName}` : "";
  const subject = `\u{1F4E8} New ${title}${subjectLead} \u2014 IvoryTusk Website`;
...
<div class="container">
  <div class="header">
    <div class="badge">\u{1F4E8} New Submission</div>
...
__name(renderGenericForm, "renderGenericForm");
__name2(renderGenericForm, "renderGenericForm");
...
//# sourceMappingURL=functionsWorker-0.008666043273135138.js.map
